...
public class BorrowUC_CTL implements ICardReaderListener, IScannerListener,
	...
	public BorrowUC_CTL(ICardReader reader, IScanner scanner, IPrinter printer,
			IDisplay display, IBookDAO bookDAO, ILoanDAO loanDAO,
			IMemberDAO memberDAO, IBorrowUI ui) {
	...
	public void cardSwiped(int memberID) {
		...
		if (!state.equals(EBorrowState.INITIALIZED)) {
			throw new RuntimeException(
					String.format(
							"BorrowUC_CTL : cardSwiped : illegal operation in state: %s",
							state));
		}
		borrower = memberDAO.getMemberByID(memberID);
		if (borrower == null) {
			ui.displayErrorMessage(String.format("Member ID %d not found",
					memberID));
			return;
		}
	...
	public void bookScanned(int barcode) {
		...
		if (state != EBorrowState.SCANNING_BOOKS) {
			throw new RuntimeException(
					String.format(
							"BorrowUC_CTL : bookScanned : illegal operation in state: %s",
							state));
		}
		...
		if (book.getState() != EBookState.AVAILABLE) {
			ui.displayErrorMessage(String.format(
					"Book %d is not available: %s", book.getID(),
					book.getState()));
			return;
		}

		if (bookList.contains(book)) {
			ui.displayErrorMessage(String.format("Book %d already scanned: ",
					book.getID()));
			return;
		}
	...
	public void setState(EBorrowState state) {
		...
		case BORROWING_RESTRICTED:
			...
			scanner.setEnabled(false);
			ui.displayErrorMessage(String.format(
					"Member %d cannot borrow at this time.", borrower.getID()));
			break;
